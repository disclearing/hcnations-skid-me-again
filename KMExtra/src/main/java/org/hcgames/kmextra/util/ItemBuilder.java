...
public class ItemBuilder{
    ...
    public static ItemStack buildItem(Plugin plugin, String path){
        ...
        if(plugin.getConfig().contains(path + ".enchantments")){
            List<String> enchantments = plugin.getConfig().getStringList(path + ".enchantments");
            for(String rawEnchantment : enchantments){
                String[] enchantmentSplit = rawEnchantment.split(":");
                item.addUnsafeEnchantment(Enchantment.getByName(enchantmentSplit[0].toUpperCase()), Integer.valueOf(enchantmentSplit[1]));
            }
        }
