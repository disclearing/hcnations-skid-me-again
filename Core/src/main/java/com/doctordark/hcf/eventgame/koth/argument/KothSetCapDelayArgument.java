...
public class KothSetCapDelayArgument extends CommandArgument{
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args){
        ...
        if(captureZone == null){
            sender.sendMessage(ChatColor.RED + kothFaction.getFormattedName(sender) + ChatColor.RED + " does not have a capture zone.");
            return true;
        }
        ...
        sender.sendMessage(ChatColor.YELLOW + "Set the capture delay of KOTH arena " +
                ChatColor.WHITE + kothFaction.getFormattedName(sender) + ChatColor.YELLOW + " to " +
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args){
        ...
        return plugin.getFactions().getFactionManager().getFactions().stream().filter(faction -> faction instanceof KothFaction).map(Faction::getName).collect(Collectors.toList());
    }
}
