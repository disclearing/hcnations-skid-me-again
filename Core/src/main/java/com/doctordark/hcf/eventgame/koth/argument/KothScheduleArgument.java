    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args){
        ...
        for(Map.Entry<LocalDateTime, String> entry : scheduleMap.entrySet()){
            ...
            if(scheduleDateTime.isAfter(now)){ // only show the events that haven't been scheduled yet.
                ...
                if(dayDifference != 0){
                    continue; // only show events today.
                }

                long remainingMillis = now.until(scheduleDateTime, ChronoUnit.MILLIS);
                shownEvents.add("  " + ChatColor.GREEN + WordUtils.capitalize(entry.getValue()) + ": " + ChatColor.GOLD + DurationFormatUtils.formatDuration(remainingMillis, TIME_UNTIL_PATTERN));
            }
        }
        ...
        sender.sendMessage(ChatColor.GRAY + "The server timezone is currently " + ChatColor.WHITE + plugin.getConfiguration().getServerTimeZone().getDisplayName() + ChatColor.GRAY + ".");
