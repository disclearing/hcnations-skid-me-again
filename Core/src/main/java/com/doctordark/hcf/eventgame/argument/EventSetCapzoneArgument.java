...
public class EventSetCapzoneArgument extends CommandArgument{
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args){
        ...
        if(capturableFaction instanceof FuryFaction){
            if(args.length < 3){
                sender.sendMessage(ChatColor.RED + "Usage: /" + label + ' ' + getName() + ' ' + faction.getName() + " <overworld|nether|end>");
                return true;
            }

            FuryFaction conquestFaction = (FuryFaction) capturableFaction;
            ...
            if(furyZone == null){
                ...
                sender.sendMessage(ChatColor.RED + "Did you mean?: " + HCF.COMMA_JOINER.join(FuryFaction.FuryZone.getNames()));
                return true;
            }

            if(!player.getWorld().getEnvironment().equals(furyZone.getEnvironment())){
                sender.sendMessage(ChatColor.RED + "You are in the environment " + player.getWorld().getEnvironment() +
                        ", " + args[2] + " requires " + furyZone.getEnvironment());
                return true;
            }

            captureZone = new CaptureZone(furyZone.getName(), furyZone.getColor().toString(), claim, FuryTracker.DEFAULT_CAP_MILLIS);
            conquestFaction.setZone(furyZone, captureZone);
        }else if(capturableFaction instanceof ConquestFaction){
            if(args.length < 3){
